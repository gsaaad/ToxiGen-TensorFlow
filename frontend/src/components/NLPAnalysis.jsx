  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${colors[color]} flex items-center justify-center text-white text-lg`}>
          {icon}
        </div>
        <div>
...
function ProgressBar({ value, max = 100, color = 'blue', label, showValue = true }) {
    ...
    <div className="space-y-1">
      {label && (
        <div className="flex justify-between text-sm">
          ...
          {showValue && <span className="font-medium text-gray-800">{value}%</span>}
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <StatBox label="Characters" value={stats.characters} icon="🔤" color="green" />
        <StatBox label="Sentences" value={stats.sentences} icon="📄" color="purple" />
        <StatBox label="Paragraphs" value={stats.paragraphs} icon="📑" color="orange" />
        <StatBox label="Unique Words" value={stats.uniqueWords} icon="✨" color="pink" />
        <StatBox label="Avg Word Length" value={stats.avgWordLength} icon="📏" color="teal" />
      </div>
      ...
      <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-semibold text-purple-700">{stats.uniqueWordPercentage}% unique</span>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center p-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl">
        <div className="text-center">
          ...
          <div className="text-xl font-bold text-gray-800">{sentiment.label}</div>
          <div className="text-sm text-gray-500">Score: {sentiment.score} | Comparative: {sentiment.comparative.toFixed(3)}</div>
        </div>
      </div>
      
      <div className="space-y-3">
        <ProgressBar value={sentiment.positivePercent} color="green" label="Positive" />
        <ProgressBar value={sentiment.neutralPercent} color="gray" label="Neutral" />
        <ProgressBar value={sentiment.negativePercent} color="red" label="Negative" />
      </div>
      
      {sentiment.positive.length > 0 && (
        <div className="mt-4">
          <div className="text-sm text-gray-500 mb-2">Positive words found:</div>
      ...
      {sentiment.negative.length > 0 && (
        <div className="mt-3">
          <div className="text-sm text-gray-500 mb-2">Negative words found:</div>
          <div className="flex flex-wrap gap-1">
...
function ReadabilityPanel({ readability }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xl">📖</span>
        <h3 className="text-lg font-semibold text-gray-800">Readability Metrics</h3>
      </div>
      
      <div className={`p-6 rounded-xl bg-gradient-to-br ${difficultyColors[readability.difficultyColor] || difficultyColors.gray} text-white text-center`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-gray-50 rounded-xl">
          ...
          <div className="text-2xl font-bold text-gray-800">{readability.fleschKincaid}</div>
        </div>
        <div className="p-4 bg-gray-50 rounded-xl">
          <div className="text-xs text-gray-500">Reading Ease</div>
          <div className="text-2xl font-bold text-gray-800">{readability.fleschReadingEase}</div>
        </div>
        <div className="p-4 bg-gray-50 rounded-xl">
          <div className="text-xs text-gray-500">Gunning Fog</div>
          <div className="text-2xl font-bold text-gray-800">{readability.gunningFog}</div>
        </div>
        <div className="p-4 bg-gray-50 rounded-xl">
          <div className="text-xs text-gray-500">SMOG Index</div>
          <div className="text-2xl font-bold text-gray-800">{readability.smog}</div>
        </div>
      </div>
    </div>
  );
      ...
      <div className="space-y-2">
        {keywords.map((kw, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-sm font-medium text-gray-700 w-24 truncate">{kw.word}</span>
            <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
              ...
              >
                <span className="text-xs text-white font-medium">{(kw.score * 100).toFixed(0)}%</span>
              </div>
            </div>
...
function LanguagePanel({ language }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xl">🌍</span>
        <h3 className="text-lg font-semibold text-gray-800">Language Detection</h3>
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-semibold text-gray-800">{language.confidence}%</span>
        </div>
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-1">
          {pos.tags.map((tag, i) => (
            <div key={i} className="flex flex-col items-center">
              <span className={`px-2 py-1 rounded ${tag.bg} ${tag.text} text-sm font-medium`}>
                {tag.word}
              </span>
              <span className="text-[10px] text-gray-400 mt-0.5">{tag.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {Object.entries(pos.distribution).map(([posType, data]) => (
          <div key={posType} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
            <span className="text-sm text-gray-600">{posType}</span>
            <span className="text-sm font-semibold text-gray-800">{data.percent}%</span>
          </div>
        ))}
      </div>
    </div>
  ...
  if (entities.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        ...
        <p className="text-xs mt-1">Try adding names, places, or organizations</p>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {entities.map((entity, i) => (
          <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${entity.color}`}>
              {entity.type}
            </span>
          </div>
        ))}
    ...
    <div className="space-y-4">
      ...
      {emotions.primary && (
        <div className={`p-6 rounded-xl ${emotions.primary.bg} text-center`}>
          ...
          <div className={`text-xl font-bold capitalize ${emotions.primary.text}`}>
            {emotions.primary.emotion}
          </div>
          <div className="text-sm opacity-70">Primary Emotion ({emotions.primary.percent}%)</div>
        </div>
      )}
      
      <div className="space-y-2">
        {emotions.emotions.filter(e => e.count > 0).map((emotion, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-lg w-8">{emotion.emoji}</span>
            <span className="text-sm text-gray-600 w-24 capitalize">{emotion.emotion}</span>
            ...
            <span className="text-sm text-gray-500 w-12 text-right">{emotion.percent}%</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-gray-50 rounded-xl text-center">
          <div className="text-3xl font-bold text-gray-800">{style.formalityLabel}</div>
        ...
        <div className="p-4 bg-gray-50 rounded-xl text-center">
          <div className="text-3xl font-bold text-gray-800">{style.confidenceLabel}</div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl text-center">
          <div className="text-sm font-semibold text-gray-800">{style.tone}</div>
        ...
        <div className="p-3 bg-gradient-to-br from-green-50 to-teal-50 rounded-xl text-center">
          <div className="text-sm font-semibold text-gray-800">{style.perspective}</div>
        ...
        <div className="p-3 bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl text-center">
          <div className="text-sm font-semibold text-gray-800">{style.voice.active}% Active</div>
...
export function NLPAnalysis({ text }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      {/* Tab Content */}
      <div className="p-6">
        {activeTab === 'stats' && <TextStatisticsPanel stats={analysis.stats} />}
        {activeTab === 'sentiment' && <SentimentPanel sentiment={analysis.sentiment} />}
        {activeTab === 'readability' && <ReadabilityPanel readability={analysis.readability} />}
        {activeTab === 'keywords' && <KeywordsPanel keywords={analysis.keywords} />}
        {activeTab === 'language' && <LanguagePanel language={analysis.language} />}
        {activeTab === 'pos' && <POSPanel pos={analysis.pos} />}
        {activeTab === 'ner' && <NERPanel entities={analysis.entities} />}
        {activeTab === 'emotions' && <EmotionsPanel emotions={analysis.emotions} />}
        {activeTab === 'style' && <WritingStylePanel style={analysis.style} />}
        {activeTab === 'wordcloud' && <WordCloudPanel words={analysis.wordFreq} />}
      </div>
    </div>
  );
}
