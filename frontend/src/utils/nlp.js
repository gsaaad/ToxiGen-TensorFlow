...
export function getTextStatistics(text) {
  ...
  return {
    ...
    avgWordLength: words.length ? (totalWordLength / words.length).toFixed(1) : 0,
    avgWordsPerSentence: sentences.length ? (words.length / sentences.length).toFixed(1) : 0,
    uniqueWords: uniqueWords.size,
    uniqueWordPercentage: words.length ? Math.round((uniqueWords.size / words.length) * 100) : 0,
    syllables,
    avgSyllablesPerWord: words.length ? (syllables / words.length).toFixed(2) : 0
  };
}
...
export function getPOSTags(text) {
  ...
  try {
    ...
    const tags = terms.map(term => {
      ...
      if (termTags.includes('Noun') || termTags.includes('Singular') || termTags.includes('Plural')) pos = 'Noun';
      else if (termTags.includes('Verb') || termTags.includes('PastTense') || termTags.includes('Gerund')) pos = 'Verb';
...
export function extractEntities(text) {
  ...
  try {
    ...
    doc.people().forEach(p => {
      entities.push({ text: p.text(), type: 'PERSON', icon: '👤', color: 'bg-blue-100 text-blue-700' });
    });

    // Places
    doc.places().forEach(p => {
      entities.push({ text: p.text(), type: 'LOCATION', icon: '📍', color: 'bg-green-100 text-green-700' });
    });

    // Organizations
    doc.organizations().forEach(o => {
      entities.push({ text: o.text(), type: 'ORG', icon: '🏢', color: 'bg-purple-100 text-purple-700' });
    });
    ...
    doc.match('#Value+').forEach(v => {
      ...
      if (txt.includes('$') || txt.includes('€') || txt.includes('£')) {
        entities.push({ text: txt, type: 'MONEY', icon: '💰', color: 'bg-yellow-100 text-yellow-700' });
      }
    });

    // Dates (using match patterns instead of dates() which requires plugin)
    doc.match('(#Month #Value|#Value #Month|#Year)').forEach(d => {
      entities.push({ text: d.text(), type: 'DATE', icon: '📅', color: 'bg-orange-100 text-orange-700' });
    });
...
const EMOTION_LEXICON = {
  joy: ['happy', 'joy', 'love', 'wonderful', 'great', 'amazing', 'excellent', 'fantastic', 'delighted', 'pleased', 'glad', 'cheerful', 'excited', 'thrilled'],
  sadness: ['sad', 'unhappy', 'depressed', 'miserable', 'sorry', 'grief', 'lonely', 'disappointed', 'heartbroken', 'devastated', 'gloomy', 'melancholy'],
  anger: ['angry', 'mad', 'furious', 'hate', 'annoyed', 'frustrated', 'outraged', 'hostile', 'bitter', 'enraged', 'irritated', 'livid'],
  fear: ['afraid', 'scared', 'terrified', 'anxious', 'worried', 'nervous', 'panic', 'dread', 'frightened', 'horrified', 'alarmed'],
  surprise: ['surprised', 'amazed', 'astonished', 'shocked', 'stunned', 'unexpected', 'startled', 'bewildered'],
  disgust: ['disgusted', 'gross', 'revolting', 'sick', 'awful', 'terrible', 'horrible', 'nasty', 'repulsive', 'vile'],
  trust: ['trust', 'believe', 'faith', 'confident', 'reliable', 'honest', 'loyal', 'dependable', 'secure'],
  anticipation: ['expect', 'hope', 'anticipate', 'await', 'eager', 'excited', 'looking forward', 'planning']
};
...
export function analyzeEmotions(text) {
  ...
  Object.entries(EMOTION_LEXICON).forEach(([emotion, lexicon]) => {
    ...
    words.forEach(word => {
      ...
      if (lexicon.some(lex => cleanWord.includes(lex) || lex.includes(cleanWord))) {
        count++;
      }
    });
...
export function analyzeWritingStyle(text) {
  ...
  const informalWords = ['gonna', 'wanna', 'gotta', 'yeah', 'nope', 'hey', 'ok', 'okay', 'lol', 'omg', 'btw', 'idk'];
  const formalWords = ['therefore', 'however', 'furthermore', 'consequently', 'nevertheless', 'regarding', 'concerning'];
  ...
  const hedgeWords = ['maybe', 'perhaps', 'possibly', 'might', 'could', 'seem', 'appear', 'think', 'believe', 'guess'];
  const assertiveWords = ['definitely', 'certainly', 'absolutely', 'clearly', 'obviously', 'must', 'will', 'always', 'never'];
  ...
  const thirdPerson = (text.match(/\b(he|she|it|they|him|her|them|his|their)\b/gi) || []).length;
  ...
  return {
    ...
    formalityLabel: formalityScore >= 70 ? 'Formal' : formalityScore >= 40 ? 'Neutral' : 'Informal',
    confidence: confidenceScore,
    confidenceLabel: confidenceScore >= 70 ? 'Assertive' : confidenceScore >= 40 ? 'Moderate' : 'Tentative',
    tone,
...
export function getWordFrequency(text, maxWords = 50) {
  ...
  const stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'shall', 'can', 'this', 'that', 'these', 'those', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'what', 'which', 'who', 'when', 'where', 'why', 'how', 'all', 'each', 'every', 'both', 'few', 'more', 'most', 'other', 'some', 'such', 'no', 'not', 'only', 'own', 'same', 'so', 'than', 'too', 'very', 'just', 'as']);
